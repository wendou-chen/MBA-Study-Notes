...
description: 自动生成考研每日计划。定时触发（cron 06:00）或手动运行 .scripts/auto_daily_plan.py。读取昨日完成率、当前阶段、待复习错题，调用 Codex API 生成兼容 kaoyan-countdown 插件的计划文件。
---
...
6. 调用 Codex：使用 `anthropic` SDK（需 `ANTHROPIC_API_KEY`）。
