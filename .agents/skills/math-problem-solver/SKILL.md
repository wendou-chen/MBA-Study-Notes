...
- **题目来源**：聊天图片（不读解题板）
- **动作**：
  1. 用聊天图片理解题目，准备写入文件的完整解答（严禁在对话框刷大段推导）
...
- **动作**：
  ...
  2. 直接准备完整解答，准备写入文件（绝不在聊天框输出长图推导）
...
7. **⚠️ 绝不污染聊天框，首要任务就是写文件**：严禁在你的对话框里刷屏输出长达几十行的包含步骤的 LaTeX 公式推导！你要做的是**直接静默调用文件写入工具**，把完整的「📝 题目」「💡 解题思路」和「📐 详细步骤」覆盖写入 `考研数学/解题板.md`。你的聊天回复只能是简短的一句话题眼总结和收录提示。
8. **⚠️ 多小题必须汇总答案**：当题目包含多个小题（如 (I)(II)(III) 或 (1)(2)(3)）时，各小题解答过程中可以分别给出答案，但**末尾必须有统一的 `**✅ 最终答案**` 段落，将所有小题答案汇总列出**。这是 error-collector 插件识别答案的唯一入口，缺少此段会导致收录时答案丢失。
...
## 聊天框极简回复规范（严禁刷屏）
每次完成**文件覆盖动作**后，你的**聊天回复必须遵循以下极简模式**：

1. **一句话脑回路**：「这题的题眼是看到条件X → 想到做Y（例如：缺 $x$ 二阶 ODE $\rightarrow$ 令 $y'=p$ 降阶）。」
2. **确认写入**：「✅ 完整详细步骤已写入 \`考研数学/解题板.md\` 。（如果有图片，说明原图已保留）。」
3. **速查表提示**：「如属于固定题型，请回复『收录』！」

> ⚠️ 【强行警告】永远不要在对话框中发 `第一步：...` `第二步：...`。只要是计算推导步骤，统统**只该出现**在你静默写入的 Markdown 文件内。聊天框仅作为与用户交流结论的窗口。
