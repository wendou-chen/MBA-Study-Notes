...
本文件为 Codex / Codex CLI 在此仓库中工作时提供上下文和规范。
...
.obsidian/
  plugins/
    claudian/      # Codex SDK 聊天插件（v1.3.65，成熟）
...
- **当前进度**：
  ...
  - **专业课**：信号与系统。UAV-PSO 轨迹优化可视化平台,SignalViz-Pro 仿真项目经验，大唐杯国二，Python/MATLAB 信号处理基础扎实。
  - **英语**：CET-6 501 分。
  - **竞赛**：MCM Meritorious Winner、AIC 机器人国一。

## Agent 系统

`.Codex/agents/` 下定义了 8 个专业 Agent：
...
### 解题板规范（硬性规则）
- **当用户询问解题板中的题目时**：
  - 必须直接编辑 `考研数学/解题板.md` 文件
  - 将完整的解答步骤写入解题板的"🧩 详细解答"部分
  - 不能只在对话中回复，必须更新文件内容
  - 解答格式遵循：知识点识别 → 通用框架 → 详细步骤 → 易错点 → 延伸训练
- **解题板结构**：
  - 📝 题目（图片嵌入）
  - 🏷️ 知识点/题型
  - 📌 已知条件与目标
...
- **Agent 定义**：修改 `.Codex/agents/*.md`，保留 YAML header
